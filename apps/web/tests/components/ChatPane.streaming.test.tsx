...
- A clear DESIGN.md with all generated rules.`,
  ...
  it('pins Codex update_plan snapshots as the chat task queue', () => {
    const messages: ChatMessage[] = [
      {
        id: 'assistant-1',
        role: 'assistant',
        ...
        events: [
          {
            ...
            input: {
              plan: [
                { step: 'Read chat code', status: 'completed' },
                { step: 'Render annotation card', status: 'in_progress' },
              ],
            },
          },
        ],
    ...
    expect(screen.getByText('Render annotation card')).toBeTruthy();
